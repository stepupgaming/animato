//! The Glacial Crown cast pipeline: zone aim, freeze-front travel, crown bloom
//! and light bookkeeping.
//!
//! Ports `Ability.js` (travel → impact → fade → done, front advance, dynamic
//! light) specialised with `GlacierAbility.js` — the sandbox's second **far
//! cast**: a circle planted at a floor point answered with a wall of crystal.
//! Everything is a pure function of `(seed, params, time)`:
//! [`GlacialCrown::seek_abs`] deterministically re-simulates from spawn, so
//! the pipeline can be driven by [`animato_composition::Composition`] —
//! including while paused, with params edited live (shard records store dice
//! only; metres resolve at sample time scaled by live
//! [`GlacialCrownParams::zone_radius`]).
//!
//! Renderer-owned systems from the original (GPU particles, ground decals,
//! burst shells, camera shake, screen flash, glacier / field / veil materials)
//! are intentionally *not* reimplemented here: the pipeline exposes every
//! stimulus they need ([`GlacialCrown::samples`], [`GlacialCrown::veil`],
//! [`GlacialCrown::field`], [`GlacialCrown::drain_events`],
//! [`GlacialCrown::light`], [`GlacialCrown::center`], [`GlacialCrown::open`]).
...
use crate::crown::{
    CrownRecord, CrownSample, FieldSample, VeilSample, roll_crown, sample_crown, sample_field,
    sample_veil, schedule_eruption,
};
...
pub enum GlacialEvent {
    /// Front arrived; sheet freezes out and the crown bloom begins.
    Impact,
    /// Collapse / shatter started.
    Fade,
    /// Pipeline complete.
    Done,
}

/// Dynamic-light state for the cast (ice glint shimmer).
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct GlacialLight {
    /// World x.
    pub x: f32,
    /// World y.
    pub y: f32,
...
/// One complete Glacial Crown pipeline: zone aim + dice + phase machine.
#[derive(Clone, Debug)]
pub struct GlacialCrown {
    ...
    light_boost: f32,
    phase: Phase,
    shards: Vec<CrownRecord>,
    events: Vec<GlacialEvent>,
}

impl GlacialCrown {
    /// Create an unspawned pipeline with default params.
    pub fn new() -> Self {
        Self::with_params(GlacialCrownParams::default())
    }

    /// Create an unspawned pipeline with explicit params.
    pub fn with_params(params: GlacialCrownParams) -> Self {
        Self {
            params,
            seed: 0.0,
            origin: [0.0, 0.0],
    ...
    pub fn params(&self) -> &GlacialCrownParams {
        &self.params
    }

    /// Mutable live params.
    pub fn params_mut(&mut self) -> &mut GlacialCrownParams {
        &mut self.params
    }
    ...
    /// Metres the freeze front has travelled.
    ...
    /// Seconds since the sheet started freezing (impact + fade).
    ...
    /// Sheet freeze-out amount `0..1` (0 while travelling).
    ...
    pub fn thaw(&self) -> f32 {
        if self.phase != Phase::Fade {
            return 0.0;
        }
        let hold = self.params.shatter_delay * 0.35;
        saturate(
            (self.fade_time - hold)
                / (self.params.fade_duration() - hold).max(0.05),
        )
    }

    /// Crown fade `1 → 0` through collapse.
    ...
    /// Whole-crown retract amount used by shard sink during fade.
    pub fn retract(&self) -> f32 {
        match self.phase {
            Phase::Fade => saturate(self.fade_time / self.params.fade_duration()),
            _ => 0.0,
        }
    }

    /// Shard dice for the current cast.
    pub fn shards(&self) -> &[CrownRecord] {
        &self.shards
    }
    ...
    pub fn spawn(&mut self, aim: &ZoneAimSolution, seed: u64) -> Result<(), SpawnError> {
        ...
        self.shards = roll_crown(&self.params, seed);
    ...
    /// Travelling freeze-front tip on the floor plane.
    ...
    fn on_impact(&mut self) {
        self.open_time = 0.0;
        schedule_eruption(&mut self.shards, &self.params, self.age, self.entry_angle);
        self.light_boost = self.params.light_intensity * 1.5;
        self.events.push(GlacialEvent::Impact);
    }
        ...
        match self.phase {
            ...
            Phase::Impact => {
                ...
                if t >= 1.0 {
                    ...
                    self.fade_time = 0.0;
                    self.events.push(GlacialEvent::Fade);
                }
            }
            Phase::Fade => {
                self.fade_time += dt;
                ...
                if t >= 1.0 {
                    ...
                    self.events.push(GlacialEvent::Done);
                }
            }
    ...
    /// Ice glint shimmer (Ability.js default — not a gutter).
    fn shimmer(&self) -> f32 {
        0.9 + 0.1
            * libm::sinf(self.age * 9.3)
            * libm::sinf(self.age * 3.7)
    }
    ...
    pub fn light(&self) -> GlacialLight {
    ...
    pub fn drain_events(&mut self) -> Vec<GlacialEvent> {
        core::mem::take(&mut self.events)
    }

    /// How many shards have erupted so far.
    ...
    pub fn seek_abs(&mut self, time: f32) {
        if self.phase == Phase::Idle && self.shards.is_empty() {
            return;
        }
    ...
    /// Resolve every erupted shard against live params.
    pub fn samples(&self) -> Vec<CrownSample> {
    ...
    /// Frozen sheet sample (hidden while travelling).
    pub fn field(&self) -> Option<FieldSample> {
    ...
    /// Cold-air curtain sample (hidden while travelling / fully thawed).
    pub fn veil(&self) -> Option<VeilSample> {
        if matches!(self.phase, Phase::Idle | Phase::Travel | Phase::Done) {
            return None;
        }
        sample_veil(
...
impl Default for GlacialCrown {
    fn default() -> Self {
        Self::new()
    }
}

impl Update for GlacialCrown {
    fn update(&mut self, dt: f32) -> bool {
        self.update(dt)
    }
}

impl Playable for GlacialCrown {
...
mod tests {
    ...
    fn cast() -> GlacialCrown {
        let mut crown = GlacialCrown::new();
    ...
    fn too_close_is_refused_when_min_range_set() {
        let mut crown = GlacialCrown::with_params(GlacialCrownParams {
            min_range: 3.0,
    ...
    fn full_lifecycle_reaches_done() {
        ...
        assert!(events.contains(&GlacialEvent::Impact));
        assert!(events.contains(&GlacialEvent::Fade));
        assert!(events.contains(&GlacialEvent::Done));
    }
    ...
    fn playable_trait_drives_progress() {
        ...
        assert!(Playable::as_any(&crown).is::<GlacialCrown>());
    }
